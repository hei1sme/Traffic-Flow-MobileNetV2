...
def slice_image(image_path, save_folder):
    ...
    # Define save paths in the specified folder
    filename = os.path.basename(image_path)
    name, ext = os.path.splitext(filename)
    part1_path = os.path.join(save_folder, f"{name}_part1{ext}")
    part2_path = os.path.join(save_folder, f"{name}_part2{ext}")
...
def process_folder():
    ...
    # Create SLICED folder inside the selected folder
    ...
    for file in os.listdir(folder_path):
        if file.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            slice_image(image_path, save_folder)
